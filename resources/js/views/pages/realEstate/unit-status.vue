...
export default {
  ...
  beforeRouteEnter(to, from, next) {
        next((vm) => {
            if (vm.$store.state.auth.work_flow_trees.includes('realEstate unit status')) {
                return true;
            } else {
  ...
  methods: {
    ...
    log(id) {
      if (this.mouseEnter != id) {
        ...
        this.mouseEnter = id;
        adminApi
          .get(`real-estate/unit-statuses/logs/${id}`)
          .then((res) => {
            let l = res.data.data;
            l.forEach((e) => {
    ...
    getData(page = 1) {
      ...
      adminApi
        .get(
          `real-estate/unit-statuses?page=${page}&per_page=${this.per_page}&search=${this.search}&${filter}`
        )
        .then((res) => {
          let l = res.data;
    ...
    getDataCurrentPage(page = 1) {
      if (
        ...
        adminApi
          .get(
            `real-estate/unit-statuses?page=${this.current_page}&per_page=${this.per_page}&search=${this.search}&${filter}&company_id=${this.company_id}`
          )
          .then((res) => {
            let l = res.data;
            this.unitStatuses = l.data;
      ...
      if (Array.isArray(id)) {
        Swal.fire({
          ...
          if (result.value) {
            this.isLoader = true;
            adminApi
              .post(`real-estate/unit-statuses/bulk-delete`, { ids: id })
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        Swal.fire({
          ...
          if (result.value) {
            ...
            adminApi
              .delete(`real-estate/unit-statuses/${id}`)
              .then((res) => {
                this.checkAll = [];
                this.getData();
                Swal.fire({
        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="row justify-content-between align-items-center mb-2">
              <h4 class="header-title">{{ $t("general.unitStatusTable") }}</h4>
                  ...
                  <div class="d-inline-flex align-items-center pagination-custom">
                    ...
                    <div class="d-inline-block">
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            unitStatusPagination.current_page == 1 ? 'none' : '',
                        }"
                      ...
                      <a
                        ...
                        :style="{
                          'pointer-events':
                            unitStatusPagination.last_page ==
                            unitStatusPagination.current_page
                              ? 'none'
                              : '',
                        }"
                        @click.prevent="getData(unitStatusPagination.current_page + 1)"
            ...
            <b-modal
              id="create"
              :title="getCompanyKey('unitstatus_create_form')"
              title-class="font-18"
              body-class="p-4 "
              :hide-footer="true"
              @show="resetModal"
              ...
              <table class="table table-borderless table-hover table-centered m-0">
                      ...
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            ...
                            @click="unitStatuses.sort(sortString('name'))"
                          ></i>
                          <i
                    ...
                    <th v-if="setting.name_e">
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="unitStatuses.sort(sortString('name_e'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="unitStatuses.sort(sortString('-name_e'))"
                    ...
                    <th v-if="setting.is_active">
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="unitStatuses.sort(sortString('is_active'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="unitStatuses.sort(sortString('-is_active'))"
                    ...
                    <th v-if="setting.is_default">
                      <div class="d-flex justify-content-center">
                        ...
                        <div class="arrow-sort">
                          <i
                            class="fas fa-arrow-up"
                            @click="unitStatuses.sort(sortString('is_default'))"
                          ></i>
                          <i
                            class="fas fa-arrow-down"
                            @click="unitStatuses.sort(sortString('-is_default'))"
                ...
                <tbody v-if="unitStatuses.length > 0">
                  <tr
                    ...
                    v-for="(data, index) in unitStatuses"
                    :key="data.id"
                  ...
                  >
                    ...
                    <td>
                      ...
                      <b-modal
                        ...
                        :title="getCompanyKey('unitstatus_edit_form')"
                        title-class="font-18"
                        body-class="p-4"
                        :ref="`edit-${data.id}`"
                        :hide-footer="true"
